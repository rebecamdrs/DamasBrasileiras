...
def pontuacao_placar(rosas_restantes, brancas_restantes, tela):
    ...
    texto_r_restantes = FONTE_PLACAR.render(str(rosas_restantes), True, BRANCO)
    ...
    texto_b_capturadas = FONTE_PLACAR.render(f'{12 - brancas_restantes}', True, BRANCO)
    ...
    texto_b_restantes = FONTE_PLACAR.render(str(brancas_restantes), True, BRANCO)
    ...
    texto_r_capturadas = FONTE_PLACAR.render(f'{12 - rosas_restantes}', True, BRANCO)
...
def turno(tela, cor):
    label_turno = FONTE_GRANDE.render('TURNO', True, BRANCO)
    if cor == BRANCO: 
        turno = 'BRANCO'
    else:
        turno = 'ROSA'
    base_turno = pygame.Rect(740, 265, 30, 100) # base para centralizar
    texto_turno = FONTE_GRANDE.render(turno, True, cor) # texto
